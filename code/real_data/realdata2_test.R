...
result_mse <- foreach(mode=c(1,2,3),.packages = c("psych","kernlab","npmr"),.verbose=TRUE)%:%
  ...
  write.csv(construc_pic(t,M_input,mode,S_total),paste("~/Dynamic_Matrix_Recovery/output/lions_",label[mode],"_",t,".csv",sep = ""))
...
result_mse <- foreach(mode=c(1,2,3),.packages = c("psych","kernlab","npmr"),.verbose=TRUE)%:%
  ...
  write.csv(construc_pic(t,M_input,mode,S_total),paste("~/Dynamic_Matrix_Recovery/output/baseline_lions_",label[mode],"_",t,".csv",sep = ""))
  cs_test_error_func(M_input,L_total[mode,t,,],p,q)
}
